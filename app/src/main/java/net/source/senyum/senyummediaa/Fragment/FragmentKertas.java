...
public class FragmentKertas extends Fragment {
    GridLayoutManager gridLayoutKertas,
            gridBrand;
    ...
    public FragmentKertas() {
    }

    @Nullable
    @Override
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_kertas, container, false);

        brand();
        perlengkapan();

        rvBrands = view.findViewById(R.id.rv_brand_kertas);
        rvBrands.setHasFixedSize(true);

        rvKertas = view.findViewById(R.id.rv_menu_kertas);
        rvKertas.setHasFixedSize(true);

        gridBrand = new GridLayoutManager(getContext(), 2, GridLayoutManager.HORIZONTAL, false);
        gridLayoutKertas = new GridLayoutManager(getContext(), 2, GridLayoutManager.VERTICAL, false);

        rvKertas.setLayoutManager(gridLayoutKertas);
        kertasAdapter = new MenuKertas(itemProduct, getContext());
        rvKertas.setAdapter(kertasAdapter);
